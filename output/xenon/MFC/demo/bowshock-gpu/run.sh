...
#SBATCH --job-name="mfc-bowshock-gpu"
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=4
#SBATCH --output="mfc-bowshock-gpu.out"
#SBATCH --error="mfc-bowshock-gpu.err"
...
$(printf "$TABLE_FORMAT_LINE" "Partition"    "gpu"          "Walltime"   "02:00:00")
$(printf "$TABLE_FORMAT_LINE" "Account"      "N/A"          "Nodes"      "2")
$(printf "$TABLE_FORMAT_LINE" "Job Name"     "mfc-bowshock-gpu"                        "Engine"     "batch")
...
END
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # mfc-bowshock-gpu @ /scratch/jobs/35186426864/MFC/demo/bowshock-gpu/case.py:"
    printf "$TABLE_HEADER"
...
. "/scratch/jobs/35186426864/MFC/demo/bowshock-gpu/mfc-environment.sh" acc || exit 1
...
echo
    ...
    cd '/scratch/jobs/35186426864/MFC/demo/bowshock-gpu'
    ...
    if [ $code -ne 0 ]; then
        ...
        cat >>'/scratch/jobs/35186426864/MFC/demo/bowshock-gpu/summary.yaml' <<EOL
syscheck:
...
simulation:
    ...
    grind: $(cat '/scratch/jobs/35186426864/MFC/demo/bowshock-gpu/time_data.dat' | tail -n 1 | awk '{print $NF}')
EOL
    ...
    t_post_process_start=$(python3 -c 'import time; print(time.time())')
        ...
        (set -x;                                         mpirun -np 8                             --map-by ppr:4:node                     --bind-to core                                          "/work/mfc/5.6.1-gcc13-ompi5/zen3/build/install/gpu-acc-2b8bf09f8f/bin/post_process")
    ...
    python3 "/work/mfc/5.6.1-gcc13-ompi5/zen3/toolchain/templates/include/generate_silo_series.py" '/scratch/jobs/35186426864/MFC/demo/bowshock-gpu'
    ...
    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/work/mfc/5.6.1-gcc13-ompi5/zen3/build/install/gpu-acc-2b8bf09f8f/bin/post_process$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi
...
EOL
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "Finished mfc-bowshock-gpu:"
