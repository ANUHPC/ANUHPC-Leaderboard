...
#SBATCH --job-name="mfc-scc26-p3-cpu"
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=4
#SBATCH --output="mfc-scc26-p3-cpu.out"
#SBATCH --error="mfc-scc26-p3-cpu.err"
...
$(printf "$TABLE_FORMAT_LINE" "Partition"    "cpu"          "Walltime"   "00:30:00")
$(printf "$TABLE_FORMAT_LINE" "Account"      "N/A"          "Nodes"      "1")
$(printf "$TABLE_FORMAT_LINE" "Job Name"     "mfc-scc26-p3-cpu"                        "Engine"     "batch")
...
END
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # mfc-scc26-p3-cpu @ /scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu/case.py:"
    printf "$TABLE_HEADER"
...
. "/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu/mfc-environment.sh" none || exit 1
...
echo
    ...
    cd '/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu'
    ...
    if [ $code -ne 0 ]; then
        ...
        cat >>'/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu/summary.yaml' <<EOL
syscheck:
...
simulation:
    ...
    grind: $(cat '/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu/time_data.dat' | tail -n 1 | awk '{print $NF}')
EOL
    ...
    t_post_process_start=$(python3 -c 'import time; print(time.time())')
        ...
        (set -x;                                         mpirun -np 4                             --map-by ppr:4:node                     --bind-to core                                          "/work/mfc/5.6.1-gcc13-ompi5/haswell/build/install/cpu-c084270971/bin/post_process")
    ...
    python3 "/work/mfc/5.6.1-gcc13-ompi5/haswell/toolchain/templates/include/generate_silo_series.py" '/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu'
    ...
    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/work/mfc/5.6.1-gcc13-ompi5/haswell/build/install/cpu-c084270971/bin/post_process$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi
        ...
        cat >>'/scratch/jobs/35187881911/MFC/demo/scc26-p3-cpu/summary.yaml' <<EOL
post_process:
    exec:  $(echo "$t_post_process_stop - $t_post_process_start" | bc -l)
EOL
    ...
    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "Finished mfc-scc26-p3-cpu:"
